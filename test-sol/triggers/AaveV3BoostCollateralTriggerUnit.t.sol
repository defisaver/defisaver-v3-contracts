...
import { AaveV3BoostCollateralTrigger } from
    "../../contracts/triggers-additional/AaveV3BoostCollateralTrigger.sol";
...
contract AaveV3BoostCollateralTriggerUnitTest is Test {
    AaveV3BoostCollateralTrigger checker;
    address constant USER = address(0x1234);
    address constant MARKET = address(0x2345);
    address constant POOL = address(0x3456);

    function setUp() public {
        checker = new AaveV3BoostCollateralTrigger();
    ...
    function check() internal view returns (bool) {
        return checker.isTriggered(abi.encode(USER, MARKET), "");
    }
