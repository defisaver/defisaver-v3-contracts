...
contract TestMidnightPaybackDirect is MidnightTestBase {
    ...
    function _payback(
        ...
        uint256 debtBefore = MIDNIGHT.debt(marketId, positionOwner);

        MidnightPaybackDirect.Params memory params = MidnightPaybackDirect.Params({
            marketId: marketId, onBehalf: _onBehalf, from: sender, amount: _inputAmount
        });

        wallet.execute(address(cut), executeActionCalldata(abi.encode(params), _isDirect), 0);

        assertEq(MIDNIGHT.debt(marketId, positionOwner), debtBefore - _expectedAmount);
