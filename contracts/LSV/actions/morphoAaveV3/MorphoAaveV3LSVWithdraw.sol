...
import "./helpers/MorphoAaveV3LSVHelper.sol";
...
contract MorphoAaveV3LSVWithdraw is ActionBase, MorphoAaveV3Helper, MorphoAaveV3LSVHelper, LSVUtilHelper {
    ...
    function _withdraw(Params memory _params) internal returns (uint256, bytes memory) {
        ...
        if (!_params.withdrawAsColl){
            address profitTrackerAddr = registry.getAddr(MORPHO_AAVEV3_PROFIT_TRACKER_ID);
            uint256 performanceFee = ILSVProfitTracker(profitTrackerAddr).withdraw(_params.tokenAddr, _params.amount, morphoAddress);
            address walletAddr = FeeRecipient(FEE_RECIPIENT_ADDRESS).getFeeAddr();
            _params.tokenAddr.withdrawTokens(walletAddr, performanceFee);
            _params.tokenAddr.withdrawTokens(_params.to, _params.amount - performanceFee);
