...
import "./helpers/AaveV3LSVHelper.sol";
...
contract AaveV3LSVWithdraw is ActionBase, AaveV3Helper, AaveV3LSVHelper, LSVUtilHelper{
    ...
    function _withdraw(
        ...
        if (_amount == type(uint256).max) {
            tokenBefore = tokenAddr.getBalance(address(this));
        }

        // withdraw underlying tokens from aave and send _to address
        lendingPool.withdraw(tokenAddr, _amount, address(this));


        // if the input amount is max calc. what was the exact _amount
        if (_amount == type(uint256).max) {
            _amount = tokenAddr.getBalance(address(this)) - tokenBefore;
        }
        ...
        tokenAddr.withdrawTokens(_to, _amount - performanceFee);
