...
contract SaverExchangeCore is SaverExchangeHelper, DSMath {
    ...
    function _sell(ExchangeData memory exData) internal returns (address, uint256) {
        address wrapper;
        uint256 swapedTokens;
        bool success;
        uint256 tokensLeft = exData.srcAmount;

        // if selling eth, convert to weth
        if (exData.srcAddr == KYBER_ETH_ADDRESS) {
            exData.srcAddr = ethToWethAddr(exData.srcAddr);
            IWETH(WETH_ADDRESS).deposit{value: exData.srcAmount}();
        }
        ...
        if (exData.price0x > 0) {
            ...
            uint256 ethAmount = getProtocolFee(exData.srcAddr, exData.srcAmount);
            (success, swapedTokens, tokensLeft) = takeOrder(
                exData,
                ethAmount,
        ...
        require(
            getBalance(exData.destAddr) >= wmul(exData.minPrice, exData.srcAmount),
    ...
    function saverSwap(ExchangeData memory _exData, ExchangeActionType _type)
        internal
        returns (uint256 swapedTokens)
    {
        ...
        if (_type == ExchangeActionType.SELL) {
            swapedTokens = IExchangeWrapper(_exData.wrapper).sell{value: ethValue}(
                _exData.srcAddr,
                _exData.destAddr,
                _exData.srcAmount
            );
        } else {
            swapedTokens = IExchangeWrapper(_exData.wrapper).buy{value: ethValue}(
                _exData.srcAddr,
                _exData.destAddr,
                _exData.destAmount
            );
    ...
    function unpackExchangeData(bytes memory _data)
        public
        pure
        returns (ExchangeData memory _exData)
    {
        (bytes memory part1, bytes memory part2) = abi.decode(_data, (bytes, bytes));

        (_exData.srcAddr, _exData.destAddr, _exData.srcAmount, _exData.destAmount) = abi.decode(
            part1,
