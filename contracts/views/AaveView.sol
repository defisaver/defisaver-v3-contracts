...
contract AaveView is AaveHelper, DSMath {
    ...
    function getLoanData(address _market, address _user) public view returns (LoanData memory data) {
        ...
        data = LoanData({
            ...
            collAmounts: new uint256[](reserves.length),
            borrowStableAmounts: new uint256[](reserves.length),
            borrowVariableAmounts: new uint256[](reserves.length)
        });
        ...
        returns (EstimatedRates[] memory)
    {
        ...
        for (uint256 i = 0; i < _reserveParams.length; ++i) {
            ...
            if (_reserveParams[i].isDebtAsset) {
                ...
                totalVariableDebt = _reserveParams[i].liquidityAdded >= totalVariableDebt
                    ? 0
