...
contract McdGive is ActionBase, GasBurner {
    ...
    function executeAction(
        ...
        (uint vaultId, address newOwner, bool createProxy, address mcdManager) = parseInputs(_callData);
    ...
    function _mcdGive(uint _vaultId, address _newOwner, bool _createProxy, address _mcdManager) internal returns (address newOwner) {
        newOwner = _newOwner;
        ...
        logger.Log(address(this), msg.sender, "McdGive", abi.encode(_vaultId, _newOwner));
    }
    ...
    {
        vaultId = abi.decode(_callData[0], (uint));
