...
/// @title MidnightWithdrawCollateral
contract MidnightWithdrawCollateral is ActionBase, MidnightHelper {
    ...
    function executeAction(
        ...
        (uint256 amount, bytes memory logData) = _withdrawCollateral(params);
    ...
    function executeActionDirect(bytes memory _callData) public payable override {
        Params memory params = parseInputs(_callData);
        (, bytes memory logData) = _withdrawCollateral(params);
    ...
    /*//////////////////////////////////////////////////////////////
                            ACTION LOGIC
    //////////////////////////////////////////////////////////////*/
    function _withdrawCollateral(Params memory _params) internal returns (uint256, bytes memory) {
        ...
        uint256 userCollateral =
            MIDNIGHT.collateral(_params.marketId, onBehalf, _params.collateralIndex);

        if (_params.amount > userCollateral) {
            _params.amount = userCollateral;
        }

        MIDNIGHT.withdrawCollateral(
            market, _params.collateralIndex, _params.amount, onBehalf, _params.to
        );

        bytes memory logData =
            abi.encode(_params.marketId, onBehalf, _params.to, _params.amount, token);
