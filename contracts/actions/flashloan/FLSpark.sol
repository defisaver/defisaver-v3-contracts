...
/// @title Action that gets and receives a FL from Spark
contract FLSpark is ActionBase, StrategyModel, ReentrancyGuard, FLHelper, IFlashLoanBase, DSMath {
    using SafeERC20 for IERC20;
    using TokenUtils for address;
    //Caller not spark pool
    error OnlySparkCallerError();
    //FL Taker must be this contract
    error SameCallerError();

    string constant ERR_ONLY_SPARK_CALLER = "Caller not spark pool";
    ...
    ILendingPoolAddressesProviderV2
        public constant addressesProvider = ILendingPoolAddressesProviderV2(
            SPARK_LENDING_POOL_ADDRESS_PROVIDER
    );
    ...
    function executeAction(
        ...
        uint flAmount = _flSpark(flData, recipeData);
    ...
    /// @param _flData All the amounts/tokens and related spark fl data
    ...
    /// @notice Spark callback function that formats and calls back RecipeExecutor
    function executeOperation(
        ...
        if (msg.sender != SPARK_LENDING_POOL){
            revert OnlySparkCallerError();
        }
        ...
        for (uint256 i = 0; i < _assets.length; i++) {
            ...
            _assets[i].approveToken(address(SPARK_LENDING_POOL), paybackAmount);
