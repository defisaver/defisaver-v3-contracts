...
contract LiquityV2Helper is MainnetLiquityV2Addresses, DSMath {
    ...
    function getDebtInFront(address _market, uint256 _trove) public view returns (uint256) {
        ...
        if (markets.current.unbackedDebt == 0) {
            ...
            uint256 redeemAmountFromFirstCall =
    ...
    function _getTotalAndUnbackedDebt(address _market)
        internal
        view
        returns (uint256 branchDebt, uint256 unbackedBold)
    {
    ...
    function _getTroveDebtInFrontForCurrentBranch(address _market, uint256 _troveId)
        public
        view
