    ...
    AaveV3Helper
{
    ...
    function formatRepaySub(AaveSubData memory _user)
        public
        view
        returns (StrategySub memory repaySub)
    {
        ...
        bytes memory triggerData = abi.encode(
            address(this), DEFAULT_AAVE_MARKET, uint256(_user.minRatio), uint8(RatioState.UNDER)
        );
    ...
    function formatBoostSub(AaveSubData memory _user)
        public
        view
        returns (StrategySub memory boostSub)
    {
        ...
        bytes memory triggerData = abi.encode(
            address(this), DEFAULT_AAVE_MARKET, uint256(_user.maxRatio), uint8(RatioState.OVER)
        );
    ...
    function parseSubData(bytes calldata _encodedInput)
        public
        pure
        returns (AaveSubData memory user)
    {
