...
library FluidVaultTypes {
    ...
    function requireT2orT4Vault(uint256 _vaultType) internal pure {
        if (_vaultType != T2_VAULT_TYPE && _vaultType != T4_VAULT_TYPE) {
            revert InvalidVaultType(_vaultType);
        }
    }

    function requireT1orT2Vault(uint256 _vaultType) internal pure {
        if (_vaultType != T1_VAULT_TYPE && _vaultType != T2_VAULT_TYPE) {
            revert InvalidVaultType(_vaultType);
        }
    }

    function requireT1orT3Vault(uint256 _vaultType) internal pure {
        if (_vaultType != T1_VAULT_TYPE && _vaultType != T3_VAULT_TYPE) {
            revert InvalidVaultType(_vaultType);
        }
    }

    function requireT3orT4Vault(uint256 _vaultType) internal pure {
