...
library FluidPaybackLiquidityLogic {
    ...
    function payback(FluidLiquidityModel.PaybackData memory _data) internal returns (uint256) {
        ...
        if (_data.amount > _data.position.borrow) {
            ...
            _data.amount = _data.position.borrow * 10_001 / 10_000 + 5;
            // If we pull more than necessary, we will take a snapshot and refund any dust amount.
            borrowTokenBalanceBefore =
                isEthPayback ? address(this).balance : _data.borrowToken.getBalance(address(this));
        }
        ...
        (,, int256 exactPaybackAmount) = _data.vaultType.isT1Vault()
            ? IFluidVaultT1(_data.vault).operate{ value: msgValue }(
                _data.nftId, 0, /* newColl_ */ paybackAmount, address(0) /* to_ */
            )
