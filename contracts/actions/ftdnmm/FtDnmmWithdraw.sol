...
/// @title Withdraw collateral from ftDNMM (PositionsManager.withdraw)
/// @dev ftDNMM enforces the post-withdraw health factor and wrapper circuit breaker on chain.
///      Amounts are never silently capped to CB capacity; size recipes with FtDnmmView.
contract FtDnmmWithdraw is ActionBase, FtDnmmHelper {
    using TokenUtils for address;

    /// @param asset The underlying asset to withdraw
    /// @param amount The amount to withdraw (uint.max for the whole available collateral balance)
    /// @param to The address which will receive the tokens (defaults to the wallet)
    struct Params {
        address asset;
        uint256 amount;
        address to;
    ...
    function executeAction(
        ...
        emit ActionEvent("FtDnmmWithdraw", logData);
    ...
    function executeActionDirect(bytes memory _callData) public payable virtual override {
        ...
        logger.logActionDirectEvent("FtDnmmWithdraw", logData);
    }
    ...
    function _withdraw(Params memory _params) internal returns (uint256, bytes memory) {
        if (_params.amount == type(uint256).max) {
            (uint256 avail,) = positionsManager.getBalance(address(this), _params.asset);
            _params.amount = avail;
        }

        // tokens are delivered to the wallet (msg.sender on PositionsManager)
        positionsManager.withdraw(_params.asset, _params.amount);
