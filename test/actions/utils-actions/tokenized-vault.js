...
const getVaultData = async (vaultAddress) => {
    ...
    const assetDecimals = await ethers
        .getContractAt('IERC20', assetAddress)
        .then((c) => c.decimals());
...
const tokenizedVaultAdapterTest = () =>
    describe('Tokenized-Vault-Adapter', () => {
        ...
        it('... should test deposit operation', async () => {
            for (const vaultAddress of vaults) {
                ...
                const assetAmount = Float2BN(
                    (+fetchAmountinUSDPrice(assetSymbol, ASSET_DOLLAR_AMOUNT)).toFixed(
                        assetDecimals,
                    ),
                    assetDecimals,
                );
                ...
                const minOutOrMaxIn = await vault.previewDeposit(
                    assetAmount.mul(99_99).div(1_00_00),
                );
        ...
        it('... should test mint operation', async () => {
            for (const vaultAddress of vaults) {
                ...
                // allow 1bpt slippage because of share appreciation
                const shareAmount = await vault.previewDeposit(
                    minOutOrMaxIn.mul(99_98).div(1_00_00),
                );
