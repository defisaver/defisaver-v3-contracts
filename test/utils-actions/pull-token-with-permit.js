...
describe('Pull-Token-Permit', function () {
    ...
    it('... should pull 1000 aDAI with permit - direct action', async () => {
        ...
        const approvedAmount = await aDaiContract.allowance(senderAcc.address, proxy.address);
        ...
        expect(proxyBalanceAfter.sub(proxyBalanceBefore)).to.be.gte(amount);
    });

    it('... should pull full balance of aDAI with permit - direct action', async () => {
        const amount = hre.ethers.utils.parseUnits('1000', 18);
        // this will work for years, current timestamp = 1743034755
        const deadline = '91743034755';

        const uintMax = hre.ethers.constants.MaxUint256;
        ...
        const proxyBalanceBefore = await balanceOf(ADAI_ADDR, proxy.address);
        const pullTokenAction = new dfs.actions.basic.PermitPullTokenAction(
            ADAI_ADDR, senderAcc.address, uintMax, deadline, v, r, s,
        );
        const pullTokenData = pullTokenAction.encodeForDsProxyCall()[1];
    ...
    it('... should pull 1000 DAI with permit - direct action', async () => {
        ...
        const approvedAmount = await daiContract.allowance(senderAcc.address, proxy.address);
        console.log(`approved amount: ${approvedAmount.toString()}`);

        console.log(proxyBalanceBefore.toString());
        console.log(proxyBalanceAfter.toString());

        expect(proxyBalanceAfter.sub(proxyBalanceBefore)).to.be.eq(amount);
    });
    it('... should pull full balance of DAI with permit - direct action', async () => {
        const amount = hre.ethers.utils.parseUnits('2000', 18);
        const deadline = 0;
        const uintMax = hre.ethers.constants.MaxUint256;
        ...
        const proxyBalanceBefore = await balanceOf(DAI_ADDR, proxy.address);
        const pullTokenAction = new dfs.actions.basic.PermitPullTokenAction(
            DAI_ADDR, senderAcc.address, uintMax, deadline, v, r, s,
        );
        const pullTokenData = pullTokenAction.encodeForDsProxyCall()[1];
