...
const aaveFlTest = async () => {
    describe('FL-AaveV2', function () {
        ...
        before(async () => {
            ...
            flActionContract = await hre.ethers.getContractAt('FLAction', flActionAddr);
        ...
        for (let i = 0; i < uniPoolInfo.length; i++) {
            it(`... should get a ${uniPoolInfo[i].token0} and ${uniPoolInfo[i].token1} UniV3 flash loan`, async () => {
                ...
                const fee0 = await fullMathLibrary.mulDivRoundingUp(
                    amount0, uniPoolInfo[i].fee, 1e6,
                );
                const fee1 = await fullMathLibrary.mulDivRoundingUp(
                    amount1, uniPoolInfo[i].fee, 1e6,
                );
                await setBalance(assetInfo0.address, proxy.address, fee0);
                await setBalance(assetInfo1.address, proxy.address, fee1);
