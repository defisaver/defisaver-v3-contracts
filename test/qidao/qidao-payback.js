...
const { qiDaoOpen, qiDaoSupply, qiDaoGenerate, qiDaoPayback } = require('../actions');
...
describe('QiDao-Open', () => {
    ...
    it('... should suply WETH to QiDao vault and borrow MAI', async () => {
        ...
        let vaultDebt = await vaultContract.vaultDebt(userVaultId);
        ...
        const repayAmount = hre.ethers.utils.parseUnits('1000', 18);
        await qiDaoPayback(
            proxy,
            registryId,
            userVaultId.toString(),
        ...
        vaultDebt = await vaultContract.vaultDebt(userVaultId);
        console.log(vaultDebt);
        const balanceAfterS = await balanceOf(MAI_STABLECOIN_ADDRESS, senderAcc.address);
        console.log(balanceAfterS.toString());
    });
});
