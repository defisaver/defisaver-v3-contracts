...
const softLiquidationTriggerTest = () =>
    describe('CurveUsd-Soft-Liquidation-Trigger', () => {
        ...
        it('... should trigger correctly', async () => {
            ...
            expect(
                await trigger.isTriggered(
                    '0x',
                    coder.encode(
                        ['(address,address,uint256)'],
                        [[WSTETH_MARKET, proxyAddr, percentage.sub(1)]],
                    ),
                ),
            ).to.be.eq(false);
            ...
            expect(
                await trigger.isTriggered(
                    ...
                    coder.encode(
                        ['(address,address,uint256)'],
                        [[WSTETH_MARKET, proxyAddr, percentage]],
                    ),
                ),
            ).to.be.eq(true);
        });
