...
With DeFi Saver you can manage and interact between decentralized finance protocols. By creating strategies you can create advanced actions that will be executed automatically when certain conditions are met. The code is [open source](https://github.com/DecenterApps/defisaver-v3-contracts) and runs on the Ethereum blockchain and other L2s.
...
| **Recipe**       | <p>A recipe is a series of actions that are bundled together and executed in one sequence. Actions can share return values and use them as inputs in next actions.</p><p>A recipe can be either executed immediately or through a strategy. If a flash loan action is used within a recipe, it must be the first action.</p>        |
