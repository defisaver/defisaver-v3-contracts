...
```solidity
function getSafeInfo(uint256 _safeId, bytes32 _collType) external view returns (uint256, uint256)
```

Gets Safe info (collateral, debt)
...
| _collType | bytes32 | CollType of the Safe
...
```solidity
function safeEngine() external view returns (contract ISAFEEngine)
```
...
| Name | Type | Description |
|---|---|---|
| _0 | contract ISAFEEngine | undefined

### safeManager

```solidity
function safeManager() external view returns (contract ISAFEManager)
```
...
| Name | Type | Description |
|---|---|---|
| _0 | contract ISAFEManager | undefined
