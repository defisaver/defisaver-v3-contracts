...
async function deployContract(contractName, args) {
    ...
    const prompt = await getInput(
        `You're deploying ${contractName} on ${network} at gas price of ${gasPriceSelected} gwei${
            args.nonce ? ` with nonce : ${args.nonce}` : ""
    ...
    if (gasPriceSelected > 300) {
        gasPriceWarning = await getInput(
            `You used a gas price of ${gasPriceSelected} gwei. Are you 100% sure? (Y/n)!\n`
        );
    ...
    let Contract = await hre.ethers.getContractFactory(`${contractPath}:${contractName}`);
    ...
    if (network !== "base") {
        console.log(
            `Transaction : https://${networkPrefix}${blockExplorer}.io/tx/${contract.deployTransaction.hash}`
        );
    } else {
    ...
    if (network !== "base") {
        console.log(
            `Contract deployed to: https://${networkPrefix}${blockExplorer}.io/address/${contract.address}`
        );
    } else {
