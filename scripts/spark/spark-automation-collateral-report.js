...
const PROTOCOL_ID = 'Spark';
const DEFAULT_OUTPUT_PATH = path.join(__dirname, 'spark-automation-collateral-report.txt');

const NETWORKS = {
    mainnet: {
        chainId: 1,
        title: 'MAINNET',
        collateralSymbols: ['rETH', 'ezETH', 'tBTC'],
    },
};
...
const printHelp = () => {
    console.log(`Usage: node scripts/spark/spark-automation-collateral-report.js [options]
...
const buildReport = (groups) => {
    const lines = [
        'SPARK AUTOMATION COLLATERAL REPORT',
...
const main = async () => {
    ...
    console.log(`Wrote ${groups.length} Spark V3 positions to ${outputPath}`);
};
