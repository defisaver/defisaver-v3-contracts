...
const sparkInfo = require('./spark.json');
...
async function main() {
    ...
    for (let i = 0; i < 300; i++) {
        console.log(`${i}/300`);
        const walletInfo = await view.callStatic.getInfoForAddress(sparkInfo.results[i].address);
        const supplied = sparkInfo.results[i].supply;
        const borrowed = sparkInfo.results[i].borrow;
