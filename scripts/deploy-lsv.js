...
async function main() {
    ...
    const proxyRegistry = await redeploy('LSVProxyRegistry', true);
    const profitTracker = await redeploy('LSVProfitTracker', true);
    ...
    const test1 = await redeploy('LSVSupply', true);
    const test2 = await redeploy('LSVBorrow', true);
    const test3 = await redeploy('LSVPayback', true);
    const test4 = await redeploy('LSVWithdraw', true);
    const test5 = await redeploy('LSVSell', true);
    const test6 = await redeploy('ApproveToken', true);
    const test7 = await redeploy('MorphoAaveV3SetManager', true);
    const test8 = await redeploy('LSVView', true);
