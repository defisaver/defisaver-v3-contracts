...
contract ReflexerRatioHelper is DSMath, MainnetReflexerAddresses {
    ...
    function getRatio(uint256 _safeId) public returns (uint256 ratio) {
        ...
        (address fsmAddr, uint256 safetyCRatio) = IOracleRelayer(ORACLE_RELAYER_ADDRESS).collateralTypes(collType);
        ...
        if (redemptionPrice == 0) {
            (,, uint256 safetyPrice,,,) = ISAFEEngine(SAFE_ENGINE_ADDRESS).collateralTypes(collType);
