...
const { morphoAaveV2Supply, morphoAaveV2Borrow } = require('../../../../test/utils/actions');
const {
...
} = require('../../../../test/strategies/utils/strategy-calls');
const { subMorphoAaveV2AutomationStrategy } = require('../../../../test/strategies/utils/strategy-subs');
const {
...
const { createStrategy, addBotCaller, createBundle } = require('../../../../test/strategies/utils/utils-strategies');
...
const morphoAaveV2BoostTest = () =>
    describe('Morpho-AaveV2-Boost-Strategy', function () {
        ...
        before(async () => {
            ...
            const openCollAmount = Float2BN(
                fetchAmountinUSDPrice(cAsset.symbol, C_DOllAR_AMOUNT_OPEN),
            );
            const openDebtAmount = Float2BN(
                fetchAmountinUSDPrice(dAsset.symbol, D_DOllAR_AMOUNT_OPEN),
            );
            await setBalance(cAsset.address, senderAcc.address, openCollAmount);
            await approve(cAsset.address, proxy.address);

            await morphoAaveV2Supply(proxy, cAsset.address, openCollAmount, senderAcc.address);

            await morphoAaveV2Borrow(proxy, dAsset.address, openDebtAmount, senderAcc.address);
        });
...
const morphoAaveV2RepayTest = () =>
    describe('Morpho-AaveV2-Repay-Strategy', function () {
        this.timeout(1000000);
