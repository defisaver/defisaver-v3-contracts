const { getAssetInfo, compare } = require('@defisaver/tokens');
...
const morphoMarkets = ['DAI', 'WETH', 'USDC', 'USDT', 'WBTC', 'STETH', 'CRV'];

const getTotalBorrowed = async (view, user, tokenAddress) =>
    view
        .getUserInfo(user)
        .then(({ userBalances }) =>
            userBalances.find(({ underlying }) => compare(underlying, tokenAddress)),
        )
        .then(({ borrowBalanceInP2P, borrowBalanceOnPool }) =>
            borrowBalanceInP2P.add(borrowBalanceOnPool),
        );

const getTotalSupplied = async (view, user, tokenAddress) =>
    view
        .getUserInfo(user)
        .then(
            ({ userBalances }) =>
                userBalances.find(({ underlying }) => compare(underlying, tokenAddress)) || {
                    supplyBalanceInP2P: Float2BN('0'),
                    supplyBalanceOnPool: Float2BN('0'),
                },
        )
        .then(({ supplyBalanceInP2P, supplyBalanceOnPool }) =>
            supplyBalanceInP2P.add(supplyBalanceOnPool),
        );

const testSupply = async ({ proxy, tokenAddress, amount, senderAcc, view }) => {
    ...
    const { logs } = await morphoAaveV2Supply(proxy, tokenAddress, amount, senderAcc.address).then(
        (e) => e.wait(),
    );

    const totalSupplied = await getTotalSupplied(view, proxy.address, tokenAddress).then((bn) =>
        bn.sub(supplyBefore),
    );
    ...
    const { data: logData } = logs.filter((e) =>
        e.topics.includes(ethers.utils.id('MorphoAaveV2Supply')),
    )[0];
...
const testBorrow = async ({ proxy, tokenAddress, amount, senderAcc }) => {
    const balanceBefore = await balanceOf(tokenAddress, senderAcc.address);
    const { logs } = await morphoAaveV2Borrow(proxy, tokenAddress, amount, senderAcc.address).then(
        (e) => e.wait(),
    );

    expect(
        await balanceOf(tokenAddress, senderAcc.address).then((e) => e.sub(balanceBefore)),
    ).to.be.eq(amount);

    const { data: logData } = logs.filter((e) =>
        e.topics.includes(ethers.utils.id('MorphoAaveV2Borrow')),
    )[0];
    const { amount: returnAmount } = ethers.utils.defaultAbiCoder.decode(
        [
            `(
...
const morphoAaveV2SupplyTest = (testLength) =>
    describe('Morpho-Supply-Test', function () {
        ...
        morphoMarkets.slice(0, testLength).map((market) =>
            describe(`Morpho-AaveV2-Supply ${market}`, () => {
                it(`... should supply ${market} to morpho`, async function () {
                    const tokenAddress = getAssetInfo(market).address;
                    const { isSupplyPaused, isDeprecated } = marketInfo.find(({ underlying }) =>
                        compare(underlying, tokenAddress),
                    ).pauseStatus;
...
const morphoAaveV2WithdrawTest = (testLength) =>
    describe('Morpho-Withdraw-Test', function () {
        this.timeout(1000000);
        let senderAcc;
        let proxy;
        let view;
        ...
        morphoMarkets.slice(0, testLength).map((market) =>
            describe(`Morpho-AaveV2-Withdraw ${market}`, async () => {
                it(`... should withdraw ${market} from morpho`, async () => {
                    ...
                    const withdrawn = await balanceOf(tokenAddress, senderAcc.address).then((bn) =>
                        bn.sub(balanceBefore),
                    );
...
const morphoAaveV2BorrowTest = (testLength) =>
    describe('Morpho-Borrow-Test', function () {
        this.timeout(1000000);
        let senderAcc;
        let proxy;
        let view;
        ...
        morphoMarkets.slice(0, testLength).map((market) =>
            it(`... should borrow ${market} from morpho`, async function () {
                const tokenAddress = getAssetInfo(market).address;
...
const morphoAaveV2PaybackTest = (testLength) =>
    describe('Morpho-Payback-Test', function () {
        this.timeout(1000000);
        let senderAcc;
        let proxy;
        let view;
        ...
        morphoMarkets.slice(0, testLength).map((market) =>
            describe(`Morpho-AaveV2-Payback ${market}`, () => {
                it(`... should payback ${market} to morpho`, async function () {
                    const tokenAddress = getAssetInfo(market).address;
                    const { isSupplyPaused, isBorrowPaused, isRepayPaused, isDeprecated } =
                        marketInfo.find(({ underlying }) =>
                            compare(underlying, tokenAddress),
                        ).pauseStatus;
                    ...
                    const { data: logData } = logs.filter((e) =>
                        e.topics.includes(ethers.utils.id('MorphoAaveV2Payback')),
                    )[0];
                    const { amount: returnAmount } = ethers.utils.defaultAbiCoder.decode(
                        [
                            `(
...
const morphoClaimTest = () =>
    describe('Morpho-Claim-Test', () => {
        ...
        it('... should claim rewards on behalf of eoa', async () => {
            ...
            await morphoClaim(proxy, onBehalfOf, claimable, proof);
...
const morphoFullTest = (testLength) =>
    describe('Morpho-Full-Test', () => {
