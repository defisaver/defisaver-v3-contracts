...
const { bprotocolLiquitySPDeposit, bprotocolLiquitySPWithdraw } = require('../../../test/utils/actions');
const { getLocalTokenPrice } = require('../../../test/utils/utils');
...
const testDeposit = async (proxy, lusdAmount, from, lqtyTo, uintmax = false) => {
    ...
    await bprotocolLiquitySPDeposit(
        proxy,
        uintmax ? ethers.constants.MaxUint256 : lusdAmount,
        from,
        lqtyTo,
    );

    const sharesMinted = await balanceOf(BAMMAddress, proxy.address).then((e) =>
        e.sub(sharesBefore),
    );
    const lqtyRewarded = await balanceOf(lqtyAddress, lqtyTo).then((balance) =>
        balance.sub(lqtyBalanceBefore),
    );

    console.log('BprotocolLiquitySPDeposit', {
        sharesMinted: BN2Float(sharesMinted),
...
const testWithdraw = async (proxy, shareAmount, to, lqtyTo) => {
    ...
    const lqtyBalanceBefore = await balanceOf(lqtyAddress, lqtyTo);
    await bprotocolLiquitySPWithdraw(proxy, shareAmount, to, lqtyTo);
    const wethReturned = await balanceOf(WETH_ADDRESS, to).then((balance) =>
        balance.sub(wethBalanceBefore),
    );
    const lusdReturned = await balanceOf(lusdAddress, to).then((balance) =>
        balance.sub(lusdBalanceBefore),
    );
    const lqtyRewarded = await balanceOf(lqtyAddress, lqtyTo).then((balance) =>
        balance.sub(lqtyBalanceBefore),
    );
...
const BprotocolLiquitySPDepositTest = () =>
    describe('Bprotocol-LiquitySP-Deposit-Test', () => {
        let senderAcc;
        let proxy;
        let snapshot;
...
const BprotocolLiquitySPWithdrawTest = () =>
    describe('Bprotocol-LiquitySP-Withdraw-Test', () => {
        let senderAcc;
        let proxy;
        let snapshot;
